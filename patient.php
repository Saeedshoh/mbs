...
<head>
	...
	<title><?=$myrow['last_name'].' '.$myrow['name'].' '.$myrow['partonymic'];?></title>
</head>
<body>
<div class="patient" >
	<div class="ramz">
...
Бо фармоиши Вазорати тандурустӣ ва ва ҳифзи иҷтимоии аҳолии Ҷумҳурии Тоҷикистон аз «03» октябри соли 2015, №840 тасдиқ карда шудааст
	...
	<div class="karta">
		...
		<?php
			$doctor = $myrow['doctor'];
			$res = mysqli_query($db, "SELECT name, last_name, phamiliya FROM doctors WHERE id = '$doctor'");
			$row = mysqli_fetch_array($res);
		 ?>
		<p class="nish">8. Ному насаби духтур &nbsp;&nbsp; <label class="ddoctor"><?=$row['last_name'];?> <?=$row['name'];?> <?=$row['phamiliya'];?></label></p>
